...
class MD5 implements SerializableHash<SH1State> {
  ...
  static int _ff(int x, int y, int z) {
    return ((x & y) | (~x & z)) & BinaryOps.mask32;
  }

  static int _gg(int x, int y, int z) {
    return ((x & z) | (y & ~z)) & BinaryOps.mask32;
  }

  static int _hh(int x, int y, int z) {
    return (x ^ y ^ z) & BinaryOps.mask32;
  }

  static int _ii(int x, int y, int z) {
    return (y ^ (x | ~z)) & BinaryOps.mask32;
  }
  ...
  static int _cc(
    ...
    return (BinaryOps.add32(
      BinaryOps.rotl32(
        BinaryOps.add32(BinaryOps.add32(BinaryOps.add32(a, f(x, y, z)), m), k),
        s,
      ),
      x,
    ));
  ...
  final List<int> _t = List<int>.generate(64, (i) {
