...
class RIPEMD320 extends _RIPEMD<RIPEMD320> {
...
class RIPEMD256 extends _RIPEMD<RIPEMD256> {
...
class RIPEMD160 extends _RIPEMD<RIPEMD160> {
...
class RIPEMD128 extends _RIPEMD<RIPEMD128> {
...
class _RIPEMD<T extends _RIPEMD<T>> implements SerializableHash<SH1State> {
  _RIPEMD(int length) {
  ...
  void _proccess128(List<int> chunk) {
    ...
    for (int i = 0; i < 64; i++) {
      int t = BinaryOps.add32(al, chunk[_zl[i]]);
      t = BinaryOps.add32(t, _RidempUtils.T(i, bl, cl, dl));
      t = BinaryOps.rotl32(t, _sl[i]);
      al = dl;
      dl = cl;
      cl = bl;
      bl = t;

      t = BinaryOps.add32(ar, chunk[_zr[i]]);
      t = BinaryOps.add32(t, _RidempUtils.t64(i, br, cr, dr));
      t = BinaryOps.rotl32(t, _sr[i]);
      ar = dr;
      dr = cr;
      cr = br;
      br = t;
    ...
    final int t = BinaryOps.add32(BinaryOps.add32(_state[1], cl), dr);
    _state[1] = BinaryOps.add32(BinaryOps.add32(_state[2], dl), ar);
    _state[2] = BinaryOps.add32(BinaryOps.add32(_state[3], al), br);

    _state[3] = BinaryOps.add32(BinaryOps.add32(_state[0], bl), cr);
  ...
  void _proccess320(List<int> chunk) {
    ...
    for (int i = 0; i < 80; i++) {
      ...
      t = BinaryOps.add32(t, _RidempUtils.T(i, bl, cl, dl));
      t = BinaryOps.rotl32(t, _sl[i]);
      t = BinaryOps.add32(t, el);
      al = el;
      el = dl;
      dl = BinaryOps.rotl32(cl, 10);
      cl = bl;
      bl = t;

      t = BinaryOps.add32(ar, chunk[_zr[i]]);
      t = BinaryOps.add32(t, _RidempUtils.t80(i, br, cr, dr));
      t = BinaryOps.rotl32(t, _sr[i]);
      t = BinaryOps.add32(t, er);
      ar = er;
      er = dr;
      dr = BinaryOps.rotl32(cr, 10);
      cr = br;
      br = t;
    ...
    _state[0] = BinaryOps.add32(_state[0], al);
    _state[1] = BinaryOps.add32(_state[1], bl);
    _state[2] = BinaryOps.add32(_state[2], cl);
    _state[3] = BinaryOps.add32(_state[3], dl);
    _state[4] = BinaryOps.add32(_state[4], el);
    _state[5] = BinaryOps.add32(_state[5], ar);
    _state[6] = BinaryOps.add32(_state[6], br);
    _state[7] = BinaryOps.add32(_state[7], cr);
    _state[8] = BinaryOps.add32(_state[8], dr);
    _state[9] = BinaryOps.add32(_state[9], er);
  }
  ...
  void _proccess256(List<int> chunk) {
    ...
    _state[2] = BinaryOps.add32(_state[2], cl);
    _state[3] = BinaryOps.add32(_state[3], dl);
    _state[4] = BinaryOps.add32(_state[4], ar);
    _state[5] = BinaryOps.add32(_state[5], br);
    _state[6] = BinaryOps.add32(_state[6], cr);
    _state[7] = BinaryOps.add32(_state[7], dr);
  }
  ...
  void _proccess160(List<int> chunk) {
    ...
    final int t = BinaryOps.add32(BinaryOps.add32(_state[1], cl), dr);
    _state[1] = BinaryOps.add32(BinaryOps.add32(_state[2], dl), er);
    _state[2] = BinaryOps.add32(BinaryOps.add32(_state[3], el), ar);
    _state[3] = BinaryOps.add32(BinaryOps.add32(_state[4], al), br);
    _state[4] = BinaryOps.add32(BinaryOps.add32(_state[0], bl), cr);
    _state[0] = t;
  }

  final List<int> _zl = const [
    0,
    1,
    2,
    3,
...
class _RidempUtils {
  ...
  static int T(int i, int bl, int cl, int dl) {
    ...
    if (i < 32) {
      return (G(bl, cl, dl) + 0x5a827999) & BinaryOps.mask32;
    }
    if (i < 48) {
      return (H(bl, cl, dl) + 0x6ed9eba1) & BinaryOps.mask32;
    }
    if (i < 64) {
      return (I(bl, cl, dl) + 0x8f1bbcdc) & BinaryOps.mask32;
    }
    return (J(bl, cl, dl) + 0xa953fd4e) & BinaryOps.mask32;
  }

  static int t64(int i, int br, int cr, int dr) {
    if (i < 16) {
      return (I(br, cr, dr) + 0x50a28be6) & BinaryOps.mask32;
    }
    if (i < 32) {
      return (H(br, cr, dr) + 0x5c4dd124) & BinaryOps.mask32;
    }
    if (i < 48) {
      return (G(br, cr, dr) + 0x6d703ef3) & BinaryOps.mask32;
    }
    return F(br, cr, dr);
  }
  ...
  static int t80(int i, int br, int cr, int dr) {
    if (i < 16) {
      return (J(br, cr, dr) + 0x50a28be6) & BinaryOps.mask32;
    }
    if (i < 32) {
      return (I(br, cr, dr) + 0x5c4dd124) & BinaryOps.mask32;
    }
    if (i < 48) {
      return (H(br, cr, dr) + 0x6d703ef3) & BinaryOps.mask32;
    }
    if (i < 64) {
      return (G(br, cr, dr) + 0x7a6d76e9) & BinaryOps.mask32;
    }
    return F(br, cr, dr);
  }
