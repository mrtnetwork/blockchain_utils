...
List<int>? convertBits(List<int> data, int fromBits, int toBits,
    {bool pad = true}) {
  int acc = 0; // Accumulator to store intermediate values.
  int bits = 0; // Number of bits in 'acc' that are valid.
  List<int> ret = []; // Resulting list of converted integers.
  int maxv = (1 << toBits) - 1; // Maximum value that can fit in 'toBits' bits.
  int maxAcc = (1 << (fromBits + toBits - 1)) - 1; // Maximum accumulator value.
