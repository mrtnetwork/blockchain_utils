...
class HashCodeGenerator {
  ...
  static int generateBytesHashCode(
    List<int> bytes, [
    ...
    for (final element in bytes) {
      hash ^= element;
      hash = (hash * 31) & BinaryOps.mask32;
    }
    if (optional.isNotEmpty) {
      hash = (hash ^ generateHashCode(optional)) & BinaryOps.mask32;
    }
    return hash;
  }
  ...
  static int generateHashCode(Iterable<Object?> objects) {
    ...
    for (final element in objects) {
      if (element is Iterable) {
        hash = (hash ^ generateHashCode(element)) & BinaryOps.mask32;
      } else {
        hash = (hash ^ element.hashCode) & BinaryOps.mask32;
      }
    }
    return hash;
  }
  ...
  static int generateHashCodeNew(Object? object, {bool sorting = false}) {
    ...
    if (object == null) {
      ...
      for (final entry in entries) {
        hash =
            (hash ^ generateHashCodeNew(entry.key, sorting: sorting)) &
            BinaryOps.mask32;
        hash =
            (hash ^ generateHashCodeNew(entry.value, sorting: sorting)) &
            BinaryOps.mask32;
      }
      ...
      if (sorting) {
        ...
        elements =
            elements.toList()..sort((a, b) => a.hashCode.compareTo(b.hashCode));
      }
      for (final element in elements) {
        hash =
            (hash ^ generateHashCodeNew(element, sorting: sorting)) &
            BinaryOps.mask32;
      }
    } else {
      hash = (hash ^ object.hashCode) & BinaryOps.mask32;
    }

    // Final mix step to reduce collisions
    hash = ((hash << 3) | (hash >> 29)) & BinaryOps.mask32;
    return hash;
  }
}
