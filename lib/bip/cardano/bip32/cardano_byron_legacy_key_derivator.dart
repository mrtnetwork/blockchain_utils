...
class CardanoByronLegacyKeyDerivator
  ...
  List<int> newPrivateKeyLeftPart(
    ...
    final List<int> multiPly =
        zlBytes.map((e) => (e * 8) & BinaryOps.mask8).toList();
    final mul = Ed25519Utils.add(multiPly, klBytes);
    return mul;
  }
  ...
  List<int> newPrivateKeyRightPart(List<int> zrBytes, List<int> krBytes) {
    return List<int>.generate(
      zrBytes.length,
      (index) => (zrBytes[index] + krBytes[index]) & BinaryOps.mask8,
    );
  }
  ...
  EDPoint newPublicKeyPoint(Bip32PublicKey pubKey, List<int> zlBytes) {
    ...
    final multiply = zlBytes.map((e) => (e * 8) & BinaryOps.mask8).toList();
    final BigInt zl8Int = BigintUtils.fromBytes(
      multiply,
