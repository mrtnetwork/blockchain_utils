...
abstract class Bip32KholawEd25519KeyDerivatorBase
    implements IBip32ChildKeyDerivator {
  ...
  Bip32ChildKey deriveFromSecret({
    ...
    List<int> chainCodeBytes = parent.keyData.chainCode.toBytes();

    /// Extract raw private key bytes from 'privKey'.
    final List<int> privKeyBytes = parent.raw;

    /// Extract compressed public key bytes from 'pubKey' and remove the compression flag.
    final List<int> pubKeyBytes = ctx.compressed.sublist(1);
    ...
    if (index.isHardened) {
      ...
      chainCodeBytes =
          QuickCrypto.hmacSha512HashHalves(chainCodeBytes, [
            0x01,
            ...privKeyBytes,
            ...indexBytes,
          ]).$2;
    ...
    final List<int> pRBytes = newPrivateKeyRightPart(
      zBytes.sublist(hmacHalfLen),
      privKeyBytes.sublist(hmacHalfLen),
    );
  ...
  Bip32ChildKey deriveFromPublic({
    ...
    /// Extract compressed public key bytes from 'pubKey' and remove the compression flag.
    final List<int> pubKeyBytes = parent.compressed.sublist(1);

    /// Compute 'Z' and update the chain code.
    final List<int> zBytes = QuickCrypto.hmacSha512Hash(chainCodeBytes, [
      0x02,
      ...pubKeyBytes,
      ...indexBytes,
    ]);
    ...
    const hmacHalfLen = QuickCrypto.hmacSha512DigestSize ~/ 2;
    final EDPoint newPubKeyPoint = newPublicKeyPoint(
      parent,
      zBytes.sublist(0, hmacHalfLen),
    );
