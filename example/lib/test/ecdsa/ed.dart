...
void _infinity() {
  final p = EDPoint(
      ...
      z: BigInt.one,
      t: BigInt.zero);
  assert(p.doublePoint().isInfinity);
  final p2 = EDPoint(
      curve: Curves.curveEd25519,
      x: BigInt.one,
      y: BigInt.one,
      z: BigInt.one,
      t: BigInt.zero);
  assert(p2.doublePoint().isInfinity);
}
...
void _infinityMul() {
  assert((Curves.generatorED25519 * Curves.generatorED25519.order!).isInfinity);
}
...
void _mulByZero() {
  assert((Curves.generatorED25519 * BigInt.zero).isInfinity);
}

void _testEncoding() {
  const generatorBytes =
      "5866666666666666666666666666666666666666666666666666666666666666";
  assert(Curves.generatorED25519.toBytes().toHex() == generatorBytes);
  final gn = EDPoint.fromBytes(
      curve: Curves.curveEd25519,
      data: BytesUtils.fromHexString(generatorBytes));
  assert(gn == Curves.generatorED25519);
}
