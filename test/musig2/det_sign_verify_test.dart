...
void _test() {
  test("valid sign verify", () {
    ...
    for (int j = 0; j < validateTestCase.length; j++) {
      ...
      final List<int> aggothernonce = BytesUtils.fromHexString(
        e["aggothernonce"],
      );
      final List<String> expected = (e["expected"] as List).cast();
      MuSig2DeterministicSignature sign = musig.deterministicSign(
      ...
      expect(
        BytesUtils.toHexString(sign.pubnonce, lowerCase: false),
        expected[0],
      );
      expect(
        BytesUtils.toHexString(sign.signature, lowerCase: false),
        expected[1],
      );
      sign = musigConst.deterministicSign(
        aggotherNonce: aggothernonce,
        msg: msg,
        sk: secretKey,
        publicKeys: keys,
