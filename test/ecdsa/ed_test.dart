...
void _addAndMull() {
  assert(
    Curves.generatorED25519 + Curves.generatorED25519 ==
...
void _infinityMul() {
  expect(
    (Curves.generatorED25519 * Curves.generatorED25519.order!).isZero(),
    true,
  );
}
...
void _infinityMulAddOne() {
  expect(
    (Curves.generatorED25519 * (Curves.generatorED25519.order! + BigInt.one)) ==
        Curves.generatorED25519,
    true,
  );
}
...
void _mulByZero() {
  expect((Curves.generatorED25519 * BigInt.zero).isZero(), true);
}
...
void _testKeys() {
  final pr = EDDSAPrivateKey.fromBytes(
    generator: Curves.generatorED25519,
    secretKey: Uint8List(32),
    hashMethod: () => SHA512(),
  );
  expect(
    pr.publicKey.point.toBytes().toHex(),
