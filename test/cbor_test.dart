...
Map<CborObject, CborObject> _mapTest1 = {
  ...
  CborStringValue("mrtnetwork"): CborBigIntValue(
...
void _decodeMap() {
  ...
  expect(
    CompareUtils.iterableIsEqual(values.map((e) => e.value), valuesDec),
    true,
  );
}
...
void _decodeList() {
  ...
  expect(
    CompareUtils.iterableIsEqual(cb.value.map((e) => e.value), valuesDec),
    true,
  );
}
...
void _decodeDateTime() {
  ...
  final decode = CborStringDateValue.decode(cb.encode());
  ...
  final decode2 = CborEpochFloatValue.decode(cb2.encode());
  ...
  final decode3 = CborEpochIntValue.decode(cb3.encode());
...
void _tag() {
  ...
  cborTag = CborTagValue(
    CborListValue<CborObject>.definite([
      ...
      const CborBaseUrlValue(
        "cXdlbHF3amVsa3Fqd2VranFrd2pla2xxd2pla2xxdw==",
      ...
      CborMapValue<CborObject, CborObject>.definite({
        ...
        CborBigIntValue(
          BigInt.parse("11111111111111111111111111"),
        ): CborIntValue(5),
...
void main() {
  ...
  test("cbor encode", () {
    expect(
      CborListValue<CborObject>.definite(_testList1).encode().toHex(),
      "8400006a6d72746e6574776f726bc24a021e19e0c9bab2400000",
    );
    expect(
      CborListValue<CborObject>.inDefinite(_testList1).encode().toHex(),
      "9f00006a6d72746e6574776f726bc24a021e19e0c9bab2400000ff",
    );
    expect(
      CborMapValue.inDefinite(_mapTest1).encode().toHex(),
      "bf00006a6d72746e6574776f726bc24a021e19e0c9bab2400000ff",
    );
    expect(
      CborMapValue.definite(_mapTest1).encode().toHex(),
    ...
    expect(
      const CborIntValue(1000000000000000).encode().toHex(),
      "1b00038d7ea4c68000",
    );
    expect(
      const CborIntValue(-1000000000000000).encode().toHex(),
    ...
    expect(
      CborStringValue("MRTNETWORK").encode().toHex(),
      "6a4d52544e4554574f524b",
    );
    expect(
      CborBytesValue(
        BytesUtils.fromHexString("1b00038d7ea4c68000"),
      ).encode().toHex(),
      "491b00038d7ea4c68000",
    );
    expect(CborFloatValue(1.0).encode().toHex(), "f93c00");
    expect(
      CborFloatValue(123123123123.2).encode().toHex(),
      "fb423caab5c3b33333",
    );
    expect(
      CborDecimalFracValue(BigInt.one, BigInt.one).encode().toHex(),
      "c4820101",
    );
    expect(
      CborListValue<CborObject>.definite([
    ...
    expect(
      CborMapValue.definite({
        const CborIntValue(0): const CborIntValue(0),
      }).encode().toHex(),
      "a10000",
    );
  });
