...
void _testZip216() {
  ...
  for (final b in nonCanonicalEncodings) {
    ...
    expect(
      () => JubJubAffineNativePoint.fromBytes(encoding, zip216Enabled: true),
...
void _serializationConsistency() {
  ...
  for (int i = 0; i < v.length; i++) {
    ...
    final batchDeserialized = JubJubAffineNativePoint.fromBytes(pBytes);
    expect(p.isOnCurve(), true);
    final affine = JubJubAffineNativePoint.fromExtendedPoint(p);
    final serialized = affine.toBytes();
    final deserialize = JubJubAffineNativePoint.fromBytes(serialized);
...
void _testMulConsistency() {
  final a = JubJubNativeFr.fromBytes(
    JubJubFr([
  ...
  final b = JubJubNativeFr.fromBytes(
    JubJubFr([
  ...
  final c = JubJubNativeFr.fromBytes(
    JubJubFr([
  ...
  final pAffineNiels = JubJubAffineNativePoint.fromExtendedPoint(p).toNiels();
...
void _isIdentity() {
  final a = _eightTorsionNative.first.mulByCofactor();
  final b = _eightTorsionNative[1].mulByCofactor();
  expect(a.u, b.u);
  expect(a.v, a.z);
  expect(b.v, b.z);
  expect(a.v != b.v, true);
  ...
  for (final i in _eightTorsionNative) {
...
void _findTrial() {
  ...
  for (var i = 0; i < 255; i++) {
    ...
    JubJubAffineNativePoint? a;
    try {
      a = JubJubAffineNativePoint.fromBytes(trialBytes);
    ...
    if (a != null) {
      ...
      JubJubNativePoint b = JubJubNativePoint.fromAffinePoint(a);
      ...
      if (!b.isIdentity()) {
        ...
        expect(_fullGeneratorNative, a);
        expect(JubJubAffineNativePoint.generator(), a);
...
// _fullGeneratorNative --------------------------------------------------------------
void _findEightTorsion() {
  // Convert _fullGeneratorNative into an ExtendedPoint
  final g0 = JubJubNativePoint.fromAffinePoint(_fullGeneratorNative);
  ...
  final g = g0.multiply(b);
  expect(g.isSmallOrder(), true);

  // Iterate through expected 8-torsion points
  var cur = g;

  for (var i = 0; i < _eightTorsionNative.length; i++) {
    final expected = _eightTorsionNative[i];
    final tmp = JubJubAffineNativePoint.fromExtendedPoint(cur);
    expect(tmp, expected);
    // cur += g;
    cur = cur + g;
  }
...
void _testBatchNormalize() {
  ...
  final result0 = List.generate(
    v.length,
    (i) => JubJubAffineNativePoint.fromExtendedPoint(v[i]),
  );
...
void _extendedNielsPointIdentity() {
  expect(
    JubJubNielsNativePoint.identity().vPlusU,
    JubJubNativePoint.identity().toNiels().vPlusU,
  );
  expect(
...
void _affineNielsPointIdentity() {
  expect(
    JubJubAffineNielsNativePoint.identity().vPlusU,
    JubJubAffineNativePoint.identity().toNiels().vPlusU,
  );
  expect(
    JubJubAffineNielsNativePoint.identity().vMinusU,
    JubJubAffineNativePoint.identity().toNiels().vMinusU,
  );
  expect(
    JubJubAffineNielsNativePoint.identity().t2d,
    JubJubAffineNativePoint.identity().toNiels().t2d,
  );
}

void _isOnCurve() {
  expect(JubJubAffineNativePoint.identity().isOnCurve(), true);
}
