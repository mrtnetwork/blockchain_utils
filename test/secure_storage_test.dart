...
void main() {
  ...
  test("PBKDF2-SHA-256", () {
    ...
    final decode = Web3SecretStorageDefinationV3.decode(
      StringUtils.fromJson(js),
      "testpassword",
    );
    expect(
      BytesUtils.toHexString(decode.data),
  ...
  test("secret storage", () {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      expect(
        BytesUtils.bytesEqual(secureStorage.data, decodeWallet.data),
        true,
      );
    ...
    for (int i = 0; i < 2; i++) {
      ...
      // Decode the encoded secure storage using the password
      final decodeWallet = Web3SecretStorageDefinationV3.decode(
        secureStorage.encrypt(encoding: SecretWalletEncoding.json),
        password,
    ...
    for (int i = 0; i < 2; i++) {
      ...
      // Decode the encoded secure storage using the password
      final decodeWallet = Web3SecretStorageDefinationV3.decode(
        secureStorage.encrypt(encoding: SecretWalletEncoding.cbor),
        password,
